...
#  when "e", "s", "S", "d", "y", "Y", "z", "Z" #e, s, S, d, y, Y, z, Z
  when 101, 115, 83, 100, 121, 89, 122, 90 #e, s, S, d, y, Y, z, Z
    @logger.write("main:esSdyYzZ\r\n")
    case @task_no
    when 1
      task1_que.force_send(command)
      @logger.write("que1: #{command}\r\n")
    when 2
    when 3
    else
    end
...
#    }
    if @task_no != 0
      ...
      @logger.write("getpri: #{priority} \r\n")
    end
  when 87 #W
    @logger.write("command: W \r\n")
...
#    }
    if @task_no != 0
      ...
      if err >=0
        @logger.write("can_wup#{@task_no} returns#{err}\r\n")
      end
    end
  when 117 #u
    @logger.write("main: u \r\n")
